...
class LernSaxMailClient:
    ...
    def get_all_emails(self):
        ...
        for page in self.mail_page_data:
            self.parse_mail_page(mail_page_text=page.text)
    ...
    def send_mail(self, **kwargs):
        # receivers, cc, bcc
        receiver = kwargs.get("receiver", self.auth_client.email)
        if isinstance(receiver, str):
            receiver = [receiver]
        if not isinstance(receiver, list):
            return {"error": "wrong receiver list format"}
        cc = kwargs.get("cc", [])
        if isinstance(cc, str):
            cc = [cc]
        if not isinstance(cc, list):
            return {"error": "wrong cc list format"}
        bcc = kwargs.get("bcc", [])
        if isinstance(bcc, str):
            bcc = [bcc]
        if not isinstance(bcc, list):
            return {"error": "wrong bcc list format"}
...
if __name__ == "__main__":
    ...
    print(client.mails[0])
    client.parse_mail(client.mails[0])
    print(client.mails[0])
    print(client.mails[0].content)
    # client.find_other_mail_pages()
    # print(len(client.other_links))
    # print(client.other_links)
    # client.get_all_mail_pages()
    # client.parse_mail_page(client.mail_page_data[5])
